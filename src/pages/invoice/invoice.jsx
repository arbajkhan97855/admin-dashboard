...
export function InvoicePage() {
  ...
  const handleSendInvoiceClick = async (id) => {
    try {
      ...
      if (res.ok) {
        setTimeout(() => window.location.href = `/send_invoice/${id}` , 800);
      } else {
        toast.error(data.message || "Error creating PDF");
      }
                        ...
                        return (
                          <>
                            <tr key={item.id}>
                              ...
                              <td>{item.Email}</td>
                              <td>
                                <Link to={`/update_invoice/${item.id}`}>
                                ...
                                ||{" "}
                                <Link to={`/view_invoice/${item.id}`}>
                                  <i className="fa-regular fa-eye" />{" "}
                                </Link>{" "}
                                ||
                                <span
                                  onClick={() =>
                                    handleSendInvoiceClick(item.id)
                                  }
                                  className="trash_btn"
                                >
